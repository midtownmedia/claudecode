...
import { el, field, number, banner, stat, fmtNum, fmtDate, select } from '../ui/dom.js';
      ...
      return el('div', { class: 'card' },
        el('div', { class: 'row-between' },
          ...
          suggested && !c.vialPrice
            ...
            : null),
        el('div', { class: 'grid' },
          field('Bottle price', number(c.vialPrice ?? 0, (v) => set('vialPrice', v ?? 0), { min: 0, step: 'any' })),
          field('Bottles per order', number(c.vialsPerOrder ?? 1, (v) => set('vialsPerOrder', v ?? 1), { min: 1, step: 1 })),
          field('Shipping on that order', number(c.shipping ?? 0, (v) => set('shipping', v ?? 0), { min: 0, step: 'any' })),
          field('Bac water bottle', number(c.bacPrice ?? 0, (v) => set('bacPrice', v ?? 0), { min: 0, step: 'any' })),
          field('Bac bottle size (mL)', number(c.bacBottleMl ?? 30, (v) => set('bacBottleMl', v ?? 30), { min: 1, step: 'any' })),
          field('Syringe box price', number(c.syringeBoxPrice ?? 0, (v) => set('syringeBoxPrice', v ?? 0), { min: 0, step: 'any' })),
          field('Syringes per box', number(c.syringesPerBox ?? 100, (v) => set('syringesPerBox', v ?? 100), { min: 1, step: 1 })),
          field('Other per dose', number(c.otherPerDose ?? 0, (v) => set('otherPerDose', v ?? 0), { min: 0, step: 'any' }), 'Swabs, sharps bin')),
        el('div', { class: 'stats' },
